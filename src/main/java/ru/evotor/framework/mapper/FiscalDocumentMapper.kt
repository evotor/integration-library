...
internal object FiscalDocumentMapper {
    ...
    fun readDocumentNumber(cursor: Cursor): Long? = cursor.safeGetLong(FiscalDocumentContract.COLUMN_DOCUMENT_NUMBER)

    fun readCreationDate(bundle: Bundle?): Date? = bundle?.safeGetSerializable(KEY_CREATION_DATE)

    fun readCreationDate(cursor: Cursor): Date? = cursor.safeGetString(FiscalDocumentContract.COLUMN_CREATION_DATE)
            ?.let { dateString ->
    ...
    fun readKktRegistrationNumber(cursor: Cursor): Long? = cursor.safeGetLong(FiscalDocumentContract.COLUMN_KKT_REGISTRATION_NUMBER)

    fun readSessionNumber(bundle: Bundle?): Long? = bundle?.safeGetLong(KEY_SESSION_NUMBER)

    fun readSessionNumber(cursor: Cursor): Long? = cursor.safeGetLong(FiscalDocumentContract.COLUMN_SESSION_NUMBER)

    fun readFiscalStorageNumber(bundle: Bundle?): Long? = bundle?.safeGetLong(KEY_FISCAL_STORAGE_NUMBER)

    fun readFiscalStorageNumber(cursor: Cursor): Long? = cursor.safeGetLong(FiscalDocumentContract.COLUMN_FISCAL_STORAGE_NUMBER)

    fun readFiscalIdentifier(bundle: Bundle?): Long? = bundle?.safeGetLong(KEY_FISCAL_IDENTIFIER)

    fun readFiscalIdentifier(cursor: Cursor): Long? = cursor.safeGetLong(FiscalDocumentContract.COLUMN_FISCAL_IDENTIFIER)
