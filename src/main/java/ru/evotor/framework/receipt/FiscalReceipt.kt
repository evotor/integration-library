...
data class FiscalReceipt internal constructor(
    ...
    private class Query :
        FilterBuilder<Query, Query.SortOrder, FiscalReceipt>(FiscalReceiptContract.URI) {
        ...
        val settlementType =
            addFieldFilter<SettlementType>(FiscalReceiptContract.COLUMN_SETTLEMENT_TYPE)
        val kktRegistrationNumber =
            addFieldFilter<Long>(FiscalDocumentContract.COLUMN_KKT_REGISTRATION_NUMBER)
        val sessionNumber = addFieldFilter<Long>(FiscalDocumentContract.COLUMN_SESSION_NUMBER)
        val fiscalStorageNumber =
            addFieldFilter<Long>(FiscalDocumentContract.COLUMN_FISCAL_STORAGE_NUMBER)
        ...
        override fun getValue(cursor: Cursor<FiscalReceipt>): FiscalReceipt {
