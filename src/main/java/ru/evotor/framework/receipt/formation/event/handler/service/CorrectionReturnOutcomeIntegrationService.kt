 ...
 */
abstract class CorrectionReturnOutcomeIntegrationService : IntegrationServiceV2() {
    ...
    companion object {
         ...
         *
         * Чтобы подписать службу на получение запроса, в манифесте приложения, в элементе `action` intent-фильтра службы, укажите значение `ru.evotor.event.correction.return_outcome.PURCHASER_REQUISITES`.
         */
        const val ACTION_PURCHASER_REQUISITES = "ru.evotor.event.correction.return_outcome.PURCHASER_REQUISITES"
        const val ACTION_DISCOUNT_SCREEN_ADDITIONAL_ITEMS = "ru.evotor.event.correction.return_outcome.DISCOUNT_SCREEN_ADDITIONAL_ITEMS"

        /**
         * Запрос адреса и места расчёта для добавления в чек.
         *
         * Чтобы подписать службу на получение запроса, в манифесте приложения, в элементе `action` intent-фильтра службы, укажите значение `ru.evotor.event.correction_return_outcome.DELIVERY_REQUISITES`.
         */
        const val ACTION_DELIVERY_REQUISITES = "ru.evotor.event.correction_return_outcome.DELIVERY_REQUISITES"

        /**
         * Разрешение необходимое приложению для работы со службой [ru.evotor.framework.receipt.formation.event.handler.service.CorrectionReturnOutcomeIntegrationService].
         *
         * Чтобы выдать разрешение, в элементе `uses-permission` манифеста приложения, укажите значение `ru.evotor.permission.CORRECTION_RETURN_OUTCOME_INTEGRATION_SERVICE`.
         */
        const val PERMISSION = "ru.evotor.permission.CORRECTION_RETURN_OUTCOME_INTEGRATION_SERVICE"
    }
}
