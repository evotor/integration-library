 ...
 */
abstract class CorrectionIncomeIntegrationService : IntegrationServiceV2() {
    ...
    companion object {
         ...
         *
         * Чтобы подписать службу на получение запроса, в манифесте приложения, в элементе `action` intent-фильтра службы, укажите значение `ru.evotor.event.correction.income.PURCHASER_REQUISITES`.
         */
        const val ACTION_PURCHASER_REQUISITES = "ru.evotor.event.correction.income.PURCHASER_REQUISITES"
        const val ACTION_DISCOUNT_SCREEN_ADDITIONAL_ITEMS = "ru.evotor.event.correction.income.DISCOUNT_SCREEN_ADDITIONAL_ITEMS"

        /**
         * Запрос адреса и места расчёта для добавления в чек.
         *
         * Чтобы подписать службу на получение запроса, в манифесте приложения, в элементе `action` intent-фильтра службы, укажите значение `ru.evotor.event.correction_income.DELIVERY_REQUISITES`.
         */
        const val ACTION_DELIVERY_REQUISITES = "ru.evotor.event.correction_income.DELIVERY_REQUISITES"

        const val ACTION_INTERNET_REQUISITES = "ru.evotor.event.correction_income.INTERNET_REQUISITES"

        /**
         * Разрешение необходимое приложению для работы со службой [ru.evotor.framework.receipt.formation.event.handler.service.CorrectionIncomeIntegrationService].
         *
         * Чтобы выдать разрешение, в элементе `uses-permission` манифеста приложения, укажите значение `ru.evotor.permission.CORRECTION_INCOME_INTEGRATION_SERVICE`.
         */
        const val PERMISSION = "ru.evotor.permission.CORRECTION_INCOME_INTEGRATION_SERVICE"
    }
}
