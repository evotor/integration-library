...
import ru.evotor.framework.receipt.formation.event.AdditionalDiscountItemsEvent
...
abstract class PaybackIntegrationService : IntegrationServiceV2() {
    ...
    final override fun onEvent(action: String, bundle: Bundle) = when (action) {
        ...
        ACTION_ADDITIONAL_DISCOUNT_ITEMS -> AdditionalDiscountItemsEvent.from(bundle)?.let { handleEvent(it) }
        else -> null
    }
    ...
    @RequiresIntentAction(ACTION_ADDITIONAL_DISCOUNT_ITEMS)
    open fun handleEvent(event: AdditionalDiscountItemsEvent): IBundlable? = null

    companion object {
        const val ACTION_PURCHASER_REQUISITES = "ru.evotor.event.payback.PURCHASER_REQUISITES"
        const val ACTION_ADDITIONAL_DISCOUNT_ITEMS = "ru.evotor.event.payback.ADDITIONAL_DISCOUNT_ITEMS"
